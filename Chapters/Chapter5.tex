...
Al tener el LM afinado se procede a entrenar un clasificador. Este proceso es descrito en la sección \ref{sec:nlpprocess}. Muchos de los pasos fueron descritos en capítulos pasados --- por ejemplo la elección de la arquitectura del modelo. Muchas decisiones tomadas para este trabajo fueron basadas en la literatura disponible del campo y citadas acordemente.
...
Primero se eligió un peso para los parámetros de \textit{dropout} y después se eligió un $\gamma$ óptimo (figura \ref{fig:claslr}).
...
Muchos datos de referencia provienen del reporte de la competencia PAN17 \parencite{rangel2017overview} y proveen un contexto para la tarea del trazo de perfiles de autores. Algunas comparaciones no directas serán realizadas con resultados de la tarea de DSLCC \parencite{zampieri2017}.
